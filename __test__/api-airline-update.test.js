...
describe('PUT /api/v1/airline/', () => {
  describe('given the airline object is updated', () => {
    var initialAirline = new AirlineModel({
      ...
      country: 'Initial Test Country', id: '777'
    })
    var updatedAirline = new AirlineModel({
      name: 'Update Test Name', icao: 'UPDATETEST',
      country: 'Update Test Country', id: '777'
    })

    beforeEach(async () => {
      await initialAirline.save()
        .then(() => { console.log('test airline document inserted', initialAirline) })
    ...
    afterEach(async () => {
      await AirlineModel.removeById(initialAirline.id)
        .then(() => { console.log('test airline document deleted', initialAirline) })
