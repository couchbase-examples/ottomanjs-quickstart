...
describe('GET /api/v1/airport/list', () => {
    describe('given we get airports with skip 1, limit 1, and search text of "jo"', () => {
        ...
        var airport2 = new AirportModel({
            ...
            country: 'Updated Test Country', id: '778', faa: 'TESTFAA'
        })

        beforeEach(async () => {
            await airport1.save()
                .then(() => { console.log('test airport document inserted', airport1) })
                .catch((e) => console.log(`test airport insert failed: ${e.message}`))
            await airport2.save()
                .then(() => { console.log('test airport document inserted', airport2) })
                .catch((e) => console.log(`test airport insert failed: ${e.message}`))
            await delay(2000)
        })
        ...
        afterEach(async () => {
            await AirportModel.removeById(airport1.id)
                .then(() => { console.log('test airport document deleted', airport1) })
                .catch((e) => console.log(`test airport remove failed: ${e.message}`))
            await AirportModel.removeById(airport2.id)
                .then(() => { console.log('test airport document deleted', airport2) })
                .catch((e) => console.log(`test airport remove failed: ${e.message}`))
        })
    })
})
