...
describe('PUT /api/v1/airport/', () => {
  describe('given the airport object is updated', () => {
    ...
    beforeEach(async () => {
      await initialAirport.save()
        .then(() => { console.log('test airport document inserted', initialAirport) })
    ...
    test('should respond with status code 200 OK and updated values of document returned', async () => {
      await delay(500)
      const response = await request(app).put(`/api/v1/airport/`).send(updatedAirport)
    ...
    afterEach(async () => {
      await AirportModel.removeById(initialAirport.id)
        .then(() => { console.log('test airport document deleted', initialAirport) })
